...
class SexpTests(unittest.TestCase):
    ...
    def test_isSexp_sexp(self):
        self.assertTrue(tries.isSexp(self.thisIsASexp))

    def test_isSexp_nested_sexp(self):
        self.assertTrue(tries.isSexp(self.thisIsANestedSexp))

    def test_isSexp_not_sexp(self):
        self.assertFalse(tries.isSexp(self.thisIsNotASexp))

    def test_isSexp_list(self):
        self.assertFalse(tries.isSexp(self.thisIsAList))

    def test_isSexp_nested_list(self):
        self.assertFalse(tries.isSexp(self.thisIsANestedList))

    def test_fromSexp_sexp(self):
        self.assertEqual(tries.fromSexp(self.thisIsASexp),
                         self.thisIsAList)

    def test_fromSexp_nested_sexp(self):
        self.assertEqual(tries.fromSexp(self.thisIsANestedSexp),
                         self.thisIsANestedList)

    def test_fromSexp_not_sexp(self):
        self.assertRaises(tries.SexpressionError,
                          tries.fromSexp, self.thisIsNotASexp)

    def test_fromSexp_list(self):
        self.assertEqual(tries.fromSexp(self.thisIsAList),
                         self.thisIsAList)

    def test_fromSexp_nested_list(self):
        self.assertEqual(tries.fromSexp(self.thisIsANestedList),
                         self.thisIsANestedList)

    def test_fromSexp_something_else(self):
        self.assertRaises(tries.SexpressionError,
                          tries.fromSexp, self.thisIsSomethingElse)

    def test_toSexp_sexp(self):
        self.assertEqual(tries.toSexp(self.thisIsASexp),
                         self.thisIsASexp)

    def test_toSexp_nested_sexp(self):
        self.assertEqual(tries.toSexp(self.thisIsANestedSexp),
                         self.thisIsANestedSexp)

    def test_toSexp_not_sexp(self):
        self.assertRaises(tries.SexpressionError,
                          tries.toSexp, self.thisIsNotASexp)

    def test_toSexp_list(self):
        self.assertEqual(tries.toSexp(self.thisIsAList),
                         self.thisIsASexp)

    def test_toSexp_nested_list(self):
        self.assertEqual(tries.toSexp(self.thisIsANestedList),
                         self.thisIsANestedSexp)

    def test_toSexp_something_else(self):
        self.assertRaises(tries.SexpressionError,
                          tries.toSexp, self.thisIsSomethingElse)
