...
class FixedBridgeSplitter(Hashring):
...
class UnallocatedHolder(Hashring):
...
class BridgeSplitter(Hashring):
    ...
    def addRing(self, ring, ringname, p=1):
        ...
        if not IHashring.providedBy(ring):
            raise ValueError("ring %r doesn't implement IHashRing interface."
                             % repr(ring))
...
class FilteredBridgeSplitter(Hashring):
